...
public:
    ...
    uint64_t hashFunction(const std::string& title) {
    uint64_t hashValue = 0;
    uint64_t base = 256; // Base to encode each character's position
    uint64_t mod = 1e9 + 7; // Large prime modulus to prevent overflow

    for (char c : title) {
        if (std::isalnum(c) || std::isspace(c)) { // Consider alphanumeric and spaces
            hashValue = (hashValue * base + std::tolower(c)) % mod;
        }
    }
    return hashValue;
}
